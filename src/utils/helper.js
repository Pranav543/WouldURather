...
function noOfQuestionsAnswered(userId, questions) {
	...
	Object.keys(questions).map(
		(qid) =>
			(questions[qid].optionOne.votes.includes(userId) || questions[qid].optionTwo.votes.includes(userId)) &&
			questionsAnswered++
	);
...
function getUnansweredQuestions(loggedUser, questions) {
	return Object.keys(questions)
		.filter(
			(key) =>
				!questions[key].optionOne.votes.includes(loggedUser) &&
				!questions[key].optionTwo.votes.includes(loggedUser)
		)
